...
canonical individual named as the ID of the main canonical individual with the hathery
...
Internaly, it does something this to write the data
```{r, eval=FALSE}
prepare_snppit_infile(
  G = reorg$snppit_genos, 
  S = reorg$snppit_meta, 
  outf = tempfile()
  )
```
And then it runs it inside `system()`.

Here is what that looks like:
```{r}
run_snppit(reorg$snppit_genos, reorg$snppit_meta)
```
